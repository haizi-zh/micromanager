...
CommPortOwnershipListener {
   ...
   to reliquish the port if have it open and no longer need it.
	...
	void floatToRaw(float val,byte[] rawData)
	{
		if (val < 0)
		{
			val *= -1;
	...
	public void sendBreak() {
		...
		byte[] cmd = this.messageAreaOut.getText().getBytes();
		data[0] = '@';
		data[1] = cmd[0];
		data[2] = cmd[1];
		data[3] = cmd[2];
		if(cmd.length>3){
			char c =(char) cmd[3];
			float value = Float.valueOf(c);
			byte[] rawData = new byte[4];
			floatToRaw(value, rawData);
			data[4]  = rawData[0];
			data[5]  = rawData[1];
			data[6]  = rawData[2];
			data[7]  = rawData[3];
		...
		//    	for (byte c:data) System.out.print((char)c);
		checkSum(data);
		byte[] data1 = new byte[]{(byte)64,(byte)84,(byte)0,(byte)83,(byte)0,(byte)10,(byte)1,(byte)10,(byte)184};
		checkSum(data1);
		try {
			os.write(data1);
		} catch (IOException e) {
	...
	public void serialEvent(SerialPortEvent e) {
		...
		case SerialPortEvent.DATA_AVAILABLE:
			...
			inputBuffer.append((char)input[0]);
			inputBuffer.append((char)input[1]);
			inputBuffer.append('\t');
			inputBuffer.append(':');
			inputBuffer.append('\t');
			float value = rawToFloat(input, 2);
