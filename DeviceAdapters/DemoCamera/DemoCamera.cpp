...
int CDemoCamera::Initialize()
{
	...
	//simulate movement and nosie
	pAct = new CPropertyAction (this, &CDemoCamera::OnNosie);
	nRet = CreateProperty("Nosie", "0", MM::Float, false, pAct);
	assert(nRet == DEVICE_OK);
	pAct = new CPropertyAction (this, &CDemoCamera::OnXPosition);
	nRet = CreateProperty("XPostion", "0", MM::Float, false, pAct);
	assert(nRet == DEVICE_OK);
	pAct = new CPropertyAction (this, &CDemoCamera::OnYPosition);
	nRet = CreateProperty("YPostion", "0", MM::Float, false, pAct);
	assert(nRet == DEVICE_OK);
	pAct = new CPropertyAction (this, &CDemoCamera::OnZPosition);
	nRet = CreateProperty("ZPostion", "0", MM::Float, false, pAct);
	assert(nRet == DEVICE_OK);
	...
	nRet = CreateProperty("BitDepth", "16", MM::Integer, false, pAct);
	assert(nRet == DEVICE_OK);
 ...
 */
int CDemoCamera::OnNosie(MM::PropertyBase* pProp, MM::ActionType eAct){
	switch(eAct)
		{
		case MM::AfterSet:
		{
...
int CDemoCamera::OnXPosition(MM::PropertyBase* pProp, MM::ActionType eAct){
	switch(eAct)
		{
		case MM::AfterSet:
		{
...
int CDemoCamera::OnYPosition(MM::PropertyBase* pProp, MM::ActionType eAct){
	switch(eAct)
		{
		case MM::AfterSet:
		{
...
int CDemoCamera::OnZPosition(MM::PropertyBase* pProp, MM::ActionType eAct){
	switch(eAct)
	{
	case MM::AfterSet:
	{
		pProp->Get(zPostion_);
		GetImageFromFile(zPostion_,imageFielData_,300*300);
	}
	break;
	case MM::BeforeGet:
	{
