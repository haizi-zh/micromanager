...
void PIGCSControllerComDevice::CreateProperties()
{
	...
	// Description
	CreateProperty(MM::g_Keyword_Description, "SIGMA FINE STAGE Adapter", MM::String, true);
...
bool PIGCSControllerCom::qIDN(std::string& sIDN)
{
	...
	deviceProxy_->GCSCommandWithAnswer("\r\n?:N", answer,1);
	if (!deviceProxy_->GCSCommandWithAnswer("\r\n?:N", answer,1))
		return false;
	sIDN = answer[0];
	return true;
}
...
bool PIGCSControllerCom::IsControllerReady( BOOL* ready)
{
	...
	if (!deviceProxy_->GCSCommandWithAnswer("\r\n!:", answer, 1))
		return false;
	*ready = ((unsigned char)(answer[0][0]) == (unsigned char)('R'));
	return true;
}
...
bool PIGCSControllerCom::IsMoving(const std::string& axes, BOOL* moving)
{
	...
	if (!GetValue(answer[0], value))
		return false;
	*moving = ((unsigned char)(answer[0][0]) != (unsigned char)('R'));
	return true;
}
...
bool PIGCSControllerCom::MOV(const std::string& axis, const double* target)
{
	...
	if(target[0]>0){
		...
		double temp = abs(target[0]);
		command << "\r\nA:1-P" << temp;
	}
	deviceProxy_->SendGCSCommand( command.str() );
	if (!deviceProxy_->SendGCSCommand( command.str() ))
	{
		return false;
	}
	deviceProxy_->SendGCSCommand( "\r\nG:" );
	if (!deviceProxy_->SendGCSCommand( "\r\nG:" ))
	{
		return false;
	}
	return true;
