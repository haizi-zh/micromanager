...
/**
 * A DataProcessor thread allows for on-the-fly modification of image data
 * during acquisition.
 * 
...
public abstract class DataProcessor<E> extends Thread {
	 ...
	 * 
	 * For example, an "Identity" DataProcessor (where nothing is done to the
	 * data) would override process() thus:
	 * 
	 * @Override public void process() { produce(poll()); }
	 * TaggedImageQueue.POISON will be the last object received by polling --
	 * the process method should pass this object on unchanged.
	 */
	...
	/*
	 * Request that the data processor stop processing. The current processing
	 * event will continue, but no others will be started.
	 */
...
//		stopRequested_ = true;
	...
	/*
	 * Returns true if the DataProcessor has started up and objects are being
	...
	/*
	 * A protected method that reads the next object from the input queue.
	 */
	protected E poll() throws InterruptedException {
		while (true) {
	...
	/*
	 * A convenience method for draining all available data objects on the input
	 * queue to a collection.
	 */
